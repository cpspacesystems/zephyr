...
LOG_MODULE_DECLARE(MLX90397, CONFIG_SENSOR_LOG_LEVEL);

void mlx90397_async_fetch(struct k_work *work)
{
	int rc;
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct mlx90397_data *data = CONTAINER_OF(dwork, struct mlx90397_data, async_fetch_work);
	const struct device *dev = data->dev;
	const struct sensor_read_config *cfg =
		data->work_ctx.iodev_sqe->sqe.iodev->data;
	struct mlx90397_encoded_data *edata;
	uint32_t buf_len = sizeof(struct mlx90397_encoded_data);
	uint8_t *buf;

	rc = mlx90397_sample_fetch_internal(dev, cfg->channels->chan_type);
	if (rc != 0) {
	...
	edata = (struct mlx90397_encoded_data *)buf;

	/* buffered from submit */
	edata->header.timestamp = data->work_ctx.timestamp;
	edata->header.range_xy_val = data->work_ctx.range_xy_val;
	edata->header.range_z_val = data->work_ctx.range_z_val;
...
void mlx90397_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	rc = mlx90397_trigger_measurement_internal(dev, cfg->channels->chan_type);
	if (rc != 0) {
	...
	data->work_ctx.range_xy_val = data->range_xy_val;
	data->work_ctx.range_z_val = data->range_z_val;
